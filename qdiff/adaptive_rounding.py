...
class AdaRoundQuantizer(nn.Module):
    ...
    def init_alpha(self, x: torch.Tensor):
        # print('x, delta:', x.device, self.delta.device)      # cpu cuda:0  not initialized.... in quantipa
        # print('delta:', self.delta)          # self.delta None TODO AttributeError: 'NoneType' object has no attribute 'device', not initialized
        # print('x:', x) 
        # print('delta, x:', torch.isnan(self.delta).any(), torch.isnan(x).any())   # ok
        x_floor = torch.floor(x / self.delta)
        if self.round_mode == 'learned_hard_sigmoid':
            ...
            # print('self.alpha: ', torch.isnan(self.alpha).any())
        else:
