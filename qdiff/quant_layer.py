...
class QuantModule(nn.Module):
    ...
    def __init__(self, org_module: Union[nn.Conv2d, nn.Linear, nn.Conv1d], weight_quant_params: dict = {},
                 act_quant_params: dict = {}, logger=logging.getLogger(__name__), disable_act_quant: bool = False, act_quant_mode: str = 'qdiff'):
        ...
        # print(self.weight_quant_params, self.act_quant_params)
        # exit(0)
    ...
    def forward(self, input: torch.Tensor, split: int = 0):
        if split != 0 and self.split != 0:
            ...
            self.logger.info(f"split at {split}!")
            self.split = split
            self.set_split()
        ...
        if self.use_weight_quant:
            if self.split != 0:
                if len(self.split) == 6:
                    ...
                    weight_2 = self.weight_quantizer_1(self.weight[:, self.split[0]+self.split[1]:self.split[0]+self.split[1]+self.split[2], ...])
                    weight_3 = self.weight_quantizer_2(self.weight[:, self.split[0]+self.split[1]+self.split[2]:self.split[0]+self.split[1]+self.split[2]+self.split[3], ...])
    ...
    def set_split(self):
        if len(self.split) == 6:
            ...
            self.weight_quantizer_1 = UniformAffineQuantizer(**self.weight_quant_params)
            self.weight_quantizer_2 = UniformAffineQuantizer(**self.weight_quant_params)
            self.weight_quantizer_3 = UniformAffineQuantizer(**self.weight_quant_params)
            self.weight_quantizer_4 = UniformAffineQuantizer(**self.weight_quant_params)
        elif len(self.split) == 3:
            # print('=========== initial edge w q=========')
            self.weight_quantizer_0 = UniformAffineQuantizer(**self.weight_quant_params)
            self.weight_quantizer_1 = UniformAffineQuantizer(**self.weight_quant_params)

        elif len(self.split) == 2:
            # print('=========== initial node w q=========')
            self.weight_quantizer_0 = UniformAffineQuantizer(**self.weight_quant_params)
            
        if self.act_quant_mode == 'qdiff':
            if len(self.split) == 6:
                self.act_quantizer_0 = UniformAffineQuantizer(**self.act_quant_params)
                self.act_quantizer_1 = UniformAffineQuantizer(**self.act_quant_params)
                self.act_quantizer_2 = UniformAffineQuantizer(**self.act_quant_params)
                self.act_quantizer_3 = UniformAffineQuantizer(**self.act_quant_params)
                self.act_quantizer_4 = UniformAffineQuantizer(**self.act_quant_params)
            elif len(self.split) == 3:
                self.act_quantizer_0 = UniformAffineQuantizer(**self.act_quant_params)
                self.act_quantizer_1 = UniformAffineQuantizer(**self.act_quant_params)
            elif len(self.split) == 2:
