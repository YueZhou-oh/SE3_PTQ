...
def layer_reconstruction(cali_data, model: QuantModel, layer: QuantModule, device: str='cuda:0', #: torch.Tensor,
    ...
    # print('act_quant:', act_quant)
    # print(layer.org_weight.data)
    # print(layer.split)
    # exit(0)
    ...
    if not act_quant:
        ...
        # Use UniformAffineQuantizer to learn delta, only act_quantizer are trainable
    ...
    loss_func = LossFunction(logger, layer, round_loss=loss_mode, weight=weight,
                             max_count=iters, rec_loss=rec_loss, b_range=b_range,
                             decay_start=0, warmup=warmup, p=p)
    
    nowtime = datetime.now().strftime("%y-%m-%d-%H:%M:%S")
    logger.info(f'=========== start generating inp oup data at {nowtime}')
    # Save data before optimizing the rounding
    sio_bs = 8
    cached_inps, cached_outs = save_inp_oup_data(cali_data,
        model, layer, asym, act_quant, sio_bs, keep_gpu=False, cond=cond, is_sm=is_sm, logger=logger)
    ...
    for i in range(iters):
        ...
        # print(cur_inp.shape, out_quant.shape, cur_out.shape)   # torch.Size([32, 128]), torch.Size([32, 512]) torch.Size([32, 512])
        # exit(0)
