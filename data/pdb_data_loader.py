...
class PdbDataset(data.Dataset):
    ...
    def return_all_data_dict(self):
        ...
        #     if tmp['aatype'].shape[0] > max_length:
        #         max_length = tmp['aatype'].shape[0]
        # print(max_length)
        ...
        for idx in range(samples):
            tmp = self.__getitem__(idx)
            # s_end = tmp['aatype'].shape[0]
            for key in tmp.keys():
                if len(all_data[key].shape) > 1:
                    s_end = tmp[key].shape[0]
    ...
    def return_initializing_data_dict(self):
        ...
        for key in tmp.keys():
            value = np.array(tmp[key])
            if len(tmp[key].shape) > 0:
                init_data[key] = torch.zeros((num, max_length) + value.shape[1:])
            else:
                init_data[key] = torch.zeros(num)
            
        for idx in range(num):
            tmp = self.__getitem__(idx)
            for key in tmp.keys():
                if len(init_data[key].shape) > 1:
                    s_end = tmp[key].shape[0]
                    init_data[key][idx][:s_end] = torch.tensor(tmp[key]).clone().detach()
                else:
                    init_data[key][idx] = torch.tensor(tmp[key]).clone().detach()
        print('constructing initializing dataset...')
        return init_data
