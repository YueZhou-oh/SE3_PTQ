...
class InvariantPointAttention(nn.Module):
    ...
    def __init__(
        ...
        self.linear_rbf = Linear(20, 1)     # Debug here
...
class IpaScore(nn.Module):
    ...
    '''
        ...
        init_node_embed : [B, N, D_node]
    '''
    def forward(self, init_node_embed, edge_embed, input_feats):
        node_mask = input_feats['res_mask'].type(torch.float32)         # 1 & 0
        diffuse_mask = (1 - input_feats['fixed_mask'].type(torch.float32)) * node_mask
        edge_mask = node_mask[..., None] * node_mask[..., None, :]      # 1 & 0
        # print(node_mask[30])
        # print(edge_mask[30])
        # print(diffuse_mask[30])
        # exit(0)
        ...
        # Main trunk
        curr_rigids = self.scale_rigids(curr_rigids)    # torch.Size([1, 100]), object, does not mean only one rigid parameter
        # print(curr_rigids)  # <openfold.utils.rigid_utils.Rigid object at 0x7fe28c72b400>
        # print(curr_rigids._rots.shape)  # torch.Size([42, 485])
        # print(curr_rigids._trans.shape)     # torch.Size([42, 485, 3])
        # print(edge_embed.shape)     # torch.Size([42, 485, 485, 128])
        # exit(0)
        ...
        for b in range(self._ipa_conf.num_blocks):
            ...
            # print('edge_embed:', torch.isnan(edge_embed).any()) 
            # print('curr_rigids:', torch.isnan(curr_rigids._rots.get_rot_mats()).any()) 
            # print('node_mask:', torch.isnan(node_mask).any()) 
            ...
            # print('node_embed 1:', torch.isnan(node_embed).any()) 
            # print('ipa_embed:', torch.isnan(ipa_embed).any()) 
            # print('node_mask:', torch.isnan(node_mask).any())       # always not
            node_embed = self.trunk[f'ipa_ln_{b}'](node_embed + ipa_embed)
            # print('node_embed 2:', torch.isnan(node_embed).any())   
            seq_tfmr_in = torch.cat([
                node_embed, self.trunk[f'skip_embed_{b}'](init_node_embed)
            ], dim=-1)
            seq_tfmr_in *= node_mask[..., None]
            # print(f'seq_tfmr_in shape: {seq_tfmr_in.shape}')
            
            # import pickle
            # with open('seq_tfmr_in.pkl', 'rb') as f:
            #     seq_tfmr_in = pickle.load(f).to(node_embed.device)
            
            # with open('node_mask.pkl', 'rb') as f:
            #     node_mask = pickle.load(f).to(node_embed.device)

            # with open('./debugs/train/encoder/seq_tfmr_in.pkl', 'rb') as f:
            ...
            # print('node_embed 3:', torch.isnan(node_embed).any())  
            rigid_update = self.trunk[f'bb_update_{b}'](
                node_embed * diffuse_mask[..., None])
            # print('curr_rigids bef:', torch.isnan(curr_rigids._rots.get_quats()).any())      # false
            curr_rigids = curr_rigids.compose_q_update_vec(
                rigid_update, diffuse_mask[..., None])
            # print('diffuse mask:', torch.isnan(diffuse_mask[..., None]).any())  # not
            # print('rigid_update:', torch.isnan(rigid_update).any())
            # print('curr_rigids aft:', torch.isnan(curr_rigids._rots.get_quats()).any())  # true
