...
class Embedder(nn.Module):
    ...
    def forward(
        ...
        # print(fixed_mask) #torch.Size([1, 100, 1])
        # print(t.shape)
...
class ScoreNetwork(nn.Module):
    ...
    def forward(self, input_feats):
        ...
        bb_mask = input_feats['res_mask'].type(torch.float32)  # [B, N] 1 & 0
        fixed_mask = input_feats['fixed_mask'].type(torch.float32)      # 0
        edge_mask = bb_mask[..., None] * bb_mask[..., None, :]
        
        # import pickle
        # with open('input_feats.pkl', 'wb') as f:
        #     pickle.dump(input_feats, f)
        
        # with open('cur_inp.pkl', 'rb') as f:
        #     cur_inp = pickle.load(f)
            
        # init_node_embed, init_edge_embed = self.embedding_layer(
        #     seq_idx=cur_inp[0].to(fixed_mask.device),
        #     t=cur_inp[1].to(fixed_mask.device),
        #     fixed_mask=cur_inp[2].to(fixed_mask.device),
        #     self_conditioning_ca=cur_inp[3].to(fixed_mask.device),
        # )
        
        # with open('./debugs/train/encoder/cali_data.pkl', 'rb') as f:
        #     cali_data = pickle.load(f)
            
        # init_node_embed, init_edge_embed = self.embedding_layer(
        #     seq_idx=cali_data['seq_idx'].to(fixed_mask.device),
        #     t=cali_data['t'].to(fixed_mask.device),
        #     fixed_mask=cali_data['fixed_mask'].type(torch.float32).to(fixed_mask.device),
        #     self_conditioning_ca=cali_data['sc_ca_t'].to(fixed_mask.device),
        # )
        
        # bb_mask = cali_data['res_mask'].type(torch.float32).to(fixed_mask.device)  # [B, N] 1 & 0
        # fixed_mask = cali_data['fixed_mask'].type(torch.float32).to(fixed_mask.device)      # 0
        # edge_mask = bb_mask[..., None] * bb_mask[..., None, :]
        
        # with open('init_node_embed.pkl', 'wb') as f:
        #     pickle.dump(init_node_embed, f)
        # with open('init_edge_embed.pkl', 'wb') as f:
        #     pickle.dump(init_edge_embed, f)
        # print('ok')
        # exit(0)
        ...
        # print('=======goes here========')   # goes here during generate getinpout
        # t_tmp = input_feats['t']
        # node_name = os.path.join('debugs/orig', f'node_{t_tmp[0]}.npy')
        # edge_name = os.path.join('debugs/orig', f'edge_{t_tmp[0]}.npy')
        # # print(node_name, edge_name)
        # np.save(edge_name, edge_embed.detach().cpu().numpy())
        # np.save(node_name, node_embed.detach().cpu().numpy())
        # exit(0)

        # Run main network
        model_out = self.score_model(node_embed, edge_embed, input_feats)
        # model_out = self.score_model(node_embed, edge_embed, cali_data)
